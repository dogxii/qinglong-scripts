// 2025.9.6 Dogxi: 能够使用 需要安装 axios moment crypto 依赖
/**
 * cron: 11 18 * * *
...
async function saveFallbackMac(e) {
  try {
    ...
    return (
      console.error(":败失址地CAM用备存保".split("").reverse().join(""), e),
      !1
    );
  }
}
...
[${this.account.alias}] 状态：本次任务 PC搜索上限${
  ...
  async saveRefreshToken(s) {
    try {
      ...
      return (
        console.error(`[${this.account.alias}] 保存刷新令牌失败:`, e),
        !1
      );
    }
  }
  async getRewardsInfo() {
    try {
      ...
      return (
        console.log(`[${this.account.alias}] 获取Rewards信息失败，已重试3次`),
        0
      );
    } catch (e) {
      return (
  ...
  async getRewardsToken() {
    try {
      ...
      return (
        console.error(`[${this.account.alias}] 获取RewardsToken出错:`, e),
        0
      );
    }
  }
...
(async () => {
  ...
  if (
    ...
    if (t) {
      ...
      t = s.map((e) => {
        return (
          console.log("准备执行账号: " + e.alias),
          new MSRewards(e).start()
        );
      });
      await Promise.all(t);
    } else {
